	...
	public static function renderTag(
		...
		if (is_array($attribs)) {
			...
			foreach ($attribs as $key => $val) {
				...
				$key = $escape ? htmlspecialchars($key, ENT_NOQUOTES, 'UTF-8') : $key;
				$val = $escape ? htmlspecialchars($val, ENT_NOQUOTES, 'UTF-8') : $val;
		...
		// value
		$value = (string) $value;
		$value = $escape ? htmlspecialchars($value, ENT_NOQUOTES, 'UTF-8') : $value;
