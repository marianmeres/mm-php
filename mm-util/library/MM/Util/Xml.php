...
class Xml
{
    ...
    public static function array2xml(array $array, $rootName = "root", $indent = "  ")
    {
         ...
         * @return string
         */
        $array2xml = function ($array, $depth, $parent, $attrs = '', $pAttrs = '')
            use (&$array2xml, $indent, $n, &$rootAttrs) { // lambda recursion
            ...
            foreach ($array as $key => $value) {
                ...
                if (is_numeric($key)) {     // non-assoc
                    ...
                    if ($key == '@attributes') {
                        foreach ((array) $value as $ak => $av) { // force array
                            $attrs .= sprintf(
                                ' %s="%s"',
                                htmlspecialchars($ak), htmlspecialchars($av)
                            );
                        }
