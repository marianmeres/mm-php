 ...
 * @group mm-util
 */
class DbUtilPdoTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @var DbUtilPdo
     */
    ...
    public function setUp()
    {
        ...
        $this->dbu = new DbUtilPdo(
            json_decode(MM_UTIL_PDO_JSON_CONFIG, true)
        );
    ...
    public function testDsnModeRequiresAtLeastDriverAndDatabaseForSqlite()
    {
        ...
        DbUtilPdo::factoryResource(array());
    }
    ...
    {
        $debug = DbUtilPdo::factoryResource(array(
            'driver'   => 'sqlite',
            'database' => ':memory:',
        ), true);
    ...
    public function testDsnStringIsBuildCorrectlyForMysqlAndPgsql()
    {
        ...
        foreach ($map as $def => $driver) {
            ...
            $d = DbUtilPdo::factoryResource(array(
                'driver'   => $def,
                'database' => 'some',
                'hostname' => 'server',
                'username' => 'hoho',
    ...
    public function testSqliteForeignKeysSupportIsEnabledByDefaultAndCanBeDisabled()
    {
        ...
        if ('sqlite' == $this->dbu->getDriverName()) {
            $this->assertEquals(
                1, $this->dbu->fetchOneSql(
                    "PRAGMA foreign_keys", null, array('limit' => null)
                )
            );
        }
        ...
        $this->assertEquals(0, $db->fetchOneSql(
            "PRAGMA foreign_keys", null, array('limit' => null))
        );
        // prx($db->getQueryLog());
    }
    ...
    {
        $row = $this->dbu->fetchRow("foo", "_test", null, array('debug' => 0));
    ...
    {
        $col = $this->dbu->fetchCol("foo", "_test", null, array(
            'order_by' => 'id', 'debug' => 0
        ));
        $this->assertSame(array('bull', 'shit', 'wow'), $col);
    }
    ...
    {
        $col = $this->dbu->fetchColSql("select foo from _test", null, array(
            'order_by' => 'id', 'debug' => 0
        ));
        $this->assertSame(array('bull', 'shit', 'wow'), $col);
    }
    ...
    {
        $res = $this->dbu->fetchOne("foo", "_test", null, array(
            'order_by' => 'id', 'debug' => 0
        ));
    ...
    {
        $res = $this->dbu->fetchOneSql("select foo from _test", null, array(
            'order_by' => 'id', 'debug' => 0
        ));
        $this->assertEquals('bull', $res);
    }
    ...
    {
        $res = $this->dbu->fetchOneSql("select * from _test", array("id" => 123));
        $this->assertNull($res);
    }
    ...
    {
        $all = $this->dbu->fetchAll("*", "_test", array('id' => 3),
                array('debug' => 0));
        $this->assertInternalType('array', $all);
        $this->assertCount(1, $all);
        $this->assertEquals('wow', $all[0]['foo']);
    ...
    public function testInsertWorks()
    {
        ...
        $affectedRows = $this->dbu->insert("_test", array(
            'uid' => 'uid-foo',
        ));

        $this->assertEquals(4, $this->dbu->fetchCount("_test"));
        $this->assertEquals(1, $this->dbu->fetchCount("_test", array("uid" => "uid-foo")));
    ...
    {
        $this->assertFalse($this->dbu->insert("_test", array()));
    }

    public function testUpdateAllWorks()
    {
        $this->assertEquals(0, $this->dbu->fetchCount("_test", array("foo" => 'xyz')));
        $result = $this->dbu->update("_test", array('foo' => 'xyz'), "1=1");
        $this->assertEquals(3, $this->dbu->fetchCount("_test", array("foo" => 'xyz')));
    ...
    {
        $this->assertEquals(0, $this->dbu->fetchCount("_test", array("foo" => 'xyz')));
        $result = $this->dbu->update("_test", array('foo' => 'xyz'), "id = 1");
        $this->assertEquals(1, $this->dbu->fetchCount("_test", array("foo" => 'xyz')));
    }

    public function testUpdateHasNoEffectOnNotMatchingWhereCondition()
    {
        $this->assertEquals(0, $this->dbu->fetchCount("_test", array("foo" => 'xyz')));
        $result = $this->dbu->update("_test", array('foo' => 'xyz'), array("id" => 213));
        $this->assertEquals(0, $this->dbu->fetchCount("_test", array("foo" => 'xyz')));
    }

    public function testUpdateReturnsFalseOnEmptyData()
    {
        $this->assertFalse($this->dbu->update("_test", array(), '1=1'));
    }
    ...
    {
        $this->assertEquals(3, $this->dbu->fetchCount("_test"));
        $this->dbu->delete("_test", array('id' => 1));
    ...
    {
        $this->assertEquals(3, $this->dbu->fetchCount("_test"));
        $this->dbu->delete("_test", array('id' => 123));
        $this->assertEquals(3, $this->dbu->fetchCount("_test"));
    }
    ...
    public function testQueryLogWorks()
    {
        ...
        $this->dbu->insert("_test", array("uid" => 1));

        $this->dbu->activateQueryLog(true);
        $prepared = $this->dbu->prepare("update _test set foo = 'x' where id = :id");

        $this->dbu->insert("_test", array("uid" => 2));         // 1
        $this->dbu->insert("_test", array("uid" => 3));         // 2
        $this->dbu->delete("_test", array("uid" => 3));         // 3
        $this->dbu->fetchOne("uid", "_test");                   // 4
        $this->dbu->execute("update _test set foo = 'pfuj'");   // 5
        for ($i = 0; $i < 3; $i++) {
            $this->dbu->execute($prepared, array(":id" => $i)); // 8
        }
    ...
    public function testInactiveQueryLogIsAlwaysNullInternally()
    {
        ...
        $this->dbu->insert("_test", array("uid" => 2));
    ...
    {
        $before = $this->dbu->fetchAll('*', '_test', null, array('order_by'=>'id'));
        $this->dbu->update('_test', array(
            'foo=' => 'foo',
            // 'foo' => function(){return 'trim';},
            // 'foo' => 'trim',
        ), null, 0);
        $after = $this->dbu->fetchAll('*', '_test', null, array('order_by'=>'id'));
    ...
    public function testExternalStaticLoggerWorks()
    {
        ...
        $this->dbu->insert('_test', array('uid' => 123,'foo' => 456));
        $this->dbu->update('_test', array('foo' => 789), "id=1");
    ...
    {
        $colNameToExpectedSign = $backup = array(
            'some' =>  '=',
            'some=' =>  '=',
            'some!' =>  '<>',
            'some>' =>  '>',
